...
varexo
   eps_ii, eta_ii, eps_b, eps_i, eps_w, eps_e, eps_x, eps_a, eps_p, eps_d, eps_xie, eta_xie
   ;
...
rhosigma=0.717609347032251;
sigmabar_ii=0.00410239587693864;
...
rhosigma=0.687187840532010;%
sigmabar_ii=0.00162583177693400;%
...
zhatSS=(1-etaSS)^(-1/xiess)*zmin; %as eta=1-(zmin/zhat)^xiess;
rho_ISS=(theta-1)/theta;
rhotildeSS=nSS^(1/(theta-1))*rho_ISS;     

ztildeSS=(xiess/(xiess+1-theta))^(1/(theta-1))*zhatSS;
...
T_spreadSS=12*(R_longSS-log(1+iiSS));
...
E_premiumSS=12*(log(R_equitySS)-log(1+rirSS));
...
E_premium_fakeSS=12*(log(R_equity_fakeSS)-log(1+rirSS));
...
AlogRSS=(12*(log(1+iiSS)));
AlogpaiSS=(12*(log(paiSS)));
AlogRRSS=(12*(log(1+rirSS)));
...
[name='E.1.']          %长期债券价格
P_long=1+ckappa*(P_long(+1))*beta*M/(pai(+1));
...
[name='E.13.']        %假想股权溢价
E_premium_fake=12*(log(R_equity_fake)-log(1+rir));
...
[name='E.16.']
E_premium_cons=12*(log(R_equity_cons)-log(1+rir));
...
[name='E.18.']       %资本风险溢价
K_premium=12*(log(R_capital)-log(1+rir));
...
(gammaw/(l*w)*(pai_w(+1)*pai(+1)-STEADY_STATE(pai))*pai_w(+1)*pai(+1)*y(+1))+zeps_w=0;
...
-gammap*(pai-STEADY_STATE(pai))*pai+beta*M*(lambda(+1)/lambda)*gammap*(pai(+1)-STEADY_STATE(pai))*pai(+1)*((y(+1))/(y))+zeps_p=0;
...
[name='E.63.']          %（39）Taylor规则（参考原文，与附录不完全相同）
...
[name='E.69.']
zeps_ii=rhoii*zeps_ii(-1)+exp(sigma_eps_ii)*sigmabar_ii*eps_ii;

[name='E.70.'] 
sigma_eps_ii=(rhosigma)*sigma_eps_ii(-1)+eta_ii;
...
shock_mat(1+burnin,strmatch('eta_ii',M_.exo_names,'exact'))= 1; %1
...
tmp1        		= sqrt(12*(  max(IRF_mat(1+burnin+1:1+burnin+IRF_periods,reorderidx(22)),1e-16) ) );
tmp2                = sqrt(12*(stochastic_steady_state(:,reorderidx(22))));
IRF_mat_percent_from_SSS_eta_ii(:,reorderidx(22))	            = log(tmp1./tmp2);

tmp1        		= sqrt(12*(  max(IRF_mat(1+burnin+1:1+burnin+IRF_periods,reorderidx(23)),1e-16) ) );
tmp2                = sqrt(12*(stochastic_steady_state(:,reorderidx(23))));
