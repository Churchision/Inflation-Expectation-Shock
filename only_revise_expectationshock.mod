...
expR_DivPrcSS=exp(divtildeSS/vtildeSS)
...
expR_DivPrc=exp(divtilde/vtilde); %Exp Divident-Price Ratio
...
[name='E.10.']         %假想企业价值的欧拉方程（如果不退出的价值）
...
(gammaw/(l*w)*(pai_w(+1)*paie-STEADY_STATE(pai))*pai_w(+1)*paie*y(+1))=0;

[name='E.35.']         %（14）工资通胀率，少输了pi，后续使用时要*pi
pai_w=w/w(-1);

[name='E.36.']         %（12）企业数量运动方程，注意eta(+1)【对的】
n=(1-eta(+1))*(n(-1)+ne);

[name='E.37.']         %（21）退出企业数量，注意eta和n(-2)和ne(-1)【对的】
nx=eta*(n(-2)+ne(-1));
 
[name='E.38.']         %（6）进入企业的欧拉方程
vtilde=(ec+fe);

[name='E.39.']         %（19）进入的拥堵外部性，ne和n的时间下标有问题【对的】
ec=big_theta*(ne/n(-1))^varsigma;
 
[name='E.40.']         %（22）清算价值，多了个exp(zeps_z)【？】
lv=((1-tau)*fe-xc);

[name='E.41']          %（20）退出拥堵外部性，时间下标【对的】
...
-gammap*(pai-STEADY_STATE(pai))*pai+beta*M*(lambda(+1)/lambda)*gammap*(paie-STEADY_STATE(pai))*paie*((y(+1))/(y))=0;
...
(phidy*(1-phii))*log((ydata/(ydata(-1))));              %冲击
...
[name='E.82.'] 
invnedata=((ec+fe))*ne/n(-1)^(1/(theta-1));
...
reorderidx(19)=(strmatch('var_y',M_.endo_names,'exact'));
...
reorderidx(27)=(strmatch('var_pai',M_.endo_names,'exact'));
//-------------------------------------------------------------------------
//                          VOLATILITY SHOCK  
//-------------------------------------------------------------------------
...
tmp1        		= sqrt(max(IRF_mat(1+burnin+1:1+burnin+IRF_periods,reorderidx(19)),1e-16)  );
tmp2                = sqrt(stochastic_steady_state(:,reorderidx(19)));
IRF_mat_percent_from_SSS_eta_ii(:,reorderidx(19))	            = 100*log(tmp1./tmp2);

tmp1        		= sqrt( max(IRF_mat(1+burnin+1:1+burnin+IRF_periods,reorderidx(27)),1e-16));
tmp2                = sqrt(stochastic_steady_state(:,reorderidx(27)));
IRF_mat_percent_from_SSS_eta_ii(:,reorderidx(27))	            = 100*log(tmp1./tmp2);
